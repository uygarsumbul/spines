function allTrees = readDataset_spines_shuffleSpineVolumes(code,directory,options)
...
for kk = 1:numel(code)
  ...
  tempVolumes = features(:, 10); spines = find(tempVolumes>0); disp(numel(spines)); % 5 for ESpine, 10 for ISynapse
  %threshold = quantile(tempVolumes, 0.8); spines = find(tempVolumes>threshold);
  spineVolumes = tempVolumes(spines); features(spines, 10) = spineVolumes(randperm(numel(spineVolumes))); % SHUFFLE SPINE VOLUMES
