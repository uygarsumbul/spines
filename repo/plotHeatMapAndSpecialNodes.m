...
for kk = 1:size(xyz, 1)
  ...
  xyz1 = xyz(kk, :) + (xyz1 - xyz(kk, :))*2; % for better visibility when density is low
  xyz2 = xyz(kk, :) + (xyz2 - xyz(kk, :))*2; % for better visibilitywhen density islow
  tt   = [xyz1; xyz2];
  plot3(tt(:,1), tt(:,2), tt(:,3), 'Color', punCo,'MarkerSize',maSi+12,'Marker','.'); % for better visibilitywhen density islow
end
end
