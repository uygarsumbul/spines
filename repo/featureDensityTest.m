function featureDensityTest(spineOrIS, basalOrApical, primaryOrIntermediateOrTerminal, qq, ISonShaftSet, minLength, shuffleCount)
%spineOrIS                       = 'spine';
...
for tr = 1:numel(trees)
  ...
  sampleSTD(tr)          = max(allTrueRatios(tr, validBranches(tr, :))) - min(allTrueRatios(tr, validBranches(tr, :))); % max(allTrueRatios(tr, validBranches(tr, :))) - min(allTrueRatios(tr, validBranches(tr, :))); % std(allTrueRatios(tr, validBranches(tr, :)));
end
for tt = 1:shuffleCount
  for tr = 1:size(eventCounts, 1)
    ...
    shuffledSTD(tr, tt)         = max(shuffEventRatios(tr, 1:validBranchCounts(tr))) - min(shuffEventRatios(tr, 1:validBranchCounts(tr))); % max(shuffEventRatios(tr, 1:validBranchCounts(tr))) - min(shuffEventRatios(tr, 1:validBranchCounts(tr))); % std(shuffEventRatios(tr, 1:validBranchCounts(tr)));
  end
...
pooledSampleSTD = max(pooledTrueRatios) - min(pooledTrueRatios); % max(pooledTrueRatios) - min(pooledTrueRatios); % std(pooledTrueRatios);
for tt = 1:shuffleCount
  for tr = 1:numel(trees)
    pooledShufRatios{tt} = [pooledShufRatios{tt} allShuffles{tt}(tr, 1:validBranchCounts(tr))];
  end
  pooledShuffSTD(tt) = max(pooledShufRatios{tt}) - min(pooledShufRatios{tt}); % max(pooledShufRatios{tt}) - min(pooledShufRatios{tt}); % std(pooledShufRatios{tt});
end
...
disp(['0.05 value of the shuffled range for individual neurons: ' num2str(all5quantile)]);
disp(['0.95 value of the shuffled range for individual neurons: ' num2str(all95quantile)]);
disp(['mean value of the shuffled range for individual neurons: ' num2str(allMean)]);
disp(['true range values for individual neurons: ' num2str(sampleSTD)]);
