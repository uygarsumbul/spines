...
sy                 = {'o','o','o'};
preamble           = '~/misc/columbiaOnHabanero/spines/repo/figures/april27_branchRatePlot_';
preamble2          = '~/misc/columbiaOnHabanero/spines/repo/figures/april27_branchRatePlot_noZeros_';
fontsize           = 16;
fontname           = 'Arial';
linwid             = 1;
per                = 0; %0.05;
...
parSet{end+1} = {{'primary', 'intermediate', 'terminal'}, 'spine', 'IS'   , [-1 0 1], [0 1]   , 'apicalOblique', 1e-6, 1e-6};
parSet{end+1} = {{'primary', 'intermediate', 'terminal'}, 'spine', 'IS'   , [-1 0 1], [0 1]   , 'apicalTuft'   , 1e-6, 1e-6};
parSet{end+1} = {{'primary', 'intermediate', 'terminal'}, 'spine', 'IS'   , [-1 0 1], [0 1]   , 'apicalTrunk'  , 1e-6, 1e-6};
parSet{end+1} = {{'primary', 'intermediate', 'terminal'}, 'spine', 'IS'   , [-1 0 1], [0 1]   , 'basal'        , 0.8 , 0.8};
parSet{end+1} = {{'primary', 'intermediate', 'terminal'}, 'spine', 'IS'   , [-1 0 1], [0 1]   , 'apicalOblique', 0.8 , 0.8};
parSet{end+1} = {{'primary', 'intermediate', 'terminal'}, 'spine', 'IS'   , [-1 0 1], [0 1]   , 'apicalTuft'   , 0.8 , 0.8};
parSet{end+1} = {{'primary', 'intermediate', 'terminal'}, 'spine', 'IS'   , [-1 0 1], [0 1]   , 'apicalTrunk'  , 0.8 , 0.8};
parSet{end+1} = {{'primary', 'intermediate', 'terminal'}, 'spine', 'spine', [-1 0 1], [-1 0 1], 'basal'        , 1e-6, 0.8};
parSet{end+1} = {{'primary', 'intermediate', 'terminal'}, 'spine', 'spine', [-1 0 1], [-1 0 1], 'apicalOblique', 1e-6, 0.8};
parSet{end+1} = {{'primary', 'intermediate', 'terminal'}, 'spine', 'spine', [-1 0 1], [-1 0 1], 'apicalTuft'   , 1e-6, 0.8};
parSet{end+1} = {{'primary', 'intermediate', 'terminal'}, 'spine', 'spine', [-1 0 1], [-1 0 1], 'apicalTrunk'  , 1e-6, 0.8};
parSet{end+1} = {{'primary', 'intermediate', 'terminal'}, 'IS'   , 'IS'   , [0 1]   , [0 1]   , 'basal'        , 1e-6, 0.8};
parSet{end+1} = {{'primary', 'intermediate', 'terminal'}, 'IS'   , 'IS'   , [0 1]   , [0 1]   , 'apicalOblique', 1e-6, 0.8};
parSet{end+1} = {{'primary', 'intermediate', 'terminal'}, 'IS'   , 'IS'   , [0 1]   , [0 1]   , 'apicalTuft'   , 1e-6, 0.8};
parSet{end+1} = {{'primary', 'intermediate', 'terminal'}, 'IS'   , 'IS'   , [0 1]   , [0 1]   , 'apicalTrunk'  , 1e-6, 0.8};
...
for pS = 1:numel(parSet)
  ...
  figure(1); % ALL
  try
    coeffs = polyfit(allTT1int(1,:), allTT1int(2,:), 1); fittedX = linspace(quantile(allTT1int(1,:), trendlim), quantile(allTT1int(1,:), 1-trendlim), 200); fittedY = polyval(coeffs, fittedX); plot(fittedX, fittedY, 'Color', cc{2}, 'LineStyle', '--', 'LineWidth', 4);
    [rrr, ppp, ~, ~] = corrcoef(allTT1int(1,:), allTT1int(2,:)); disp(['r_int=' num2str(rrr(1,2)) ', p_int=' num2str(ppp(1,2))]);
  end
  try
    coeffs = polyfit(allTT1ter(1,:), allTT1ter(2,:), 1); fittedX = linspace(quantile(allTT1ter(1,:), trendlim), quantile(allTT1ter(1,:), 1-trendlim), 200); fittedY = polyval(coeffs, fittedX); plot(fittedX, fittedY, 'Color', cc{3}, 'LineStyle', '--', 'LineWidth', 4);
  ...
  xlim([quantile(allTT1(1, :), per) quantile(allTT1(1, :), 1-per)+eps]); ylim([quantile(allTT1(2, :), per) quantile(allTT1(2, :), 1-per)+eps]);
  ...
  figure(3); % NO ZEROS
  try
    coeffs = polyfit(allTT3int(1,:), allTT3int(2,:), 1); fittedX = linspace(quantile(allTT3int(1,:), trendlim), quantile(allTT3int(1,:), 1-trendlim), 200); fittedY = polyval(coeffs, fittedX); plot(fittedX, fittedY, 'Color', cc{2}, 'LineStyle', '--', 'LineWidth', 4);
    [rrr, ppp, ~, ~] = corrcoef(allTT3int(1,:), allTT3int(2,:)); disp(['r_int_noZeros=' num2str(rrr(1,2)) ', p_int_noZeros=' num2str(ppp(1,2))]);
  end
  try
    coeffs = polyfit(allTT3ter(1,:), allTT3ter(2,:), 1); fittedX = linspace(quantile(allTT3ter(1,:), trendlim), quantile(allTT3ter(1,:), 1-trendlim), 200); fittedY = polyval(coeffs, fittedX); plot(fittedX, fittedY, 'Color', cc{3}, 'LineStyle', '--', 'LineWidth', 4);
  ...
  xlim([quantile(allTT3(1, :), per) quantile(allTT3(1, :), 1-per)+eps]); ylim([quantile(allTT3(2, :), per) quantile(allTT3(2, :), 1-per)+eps]);
