...
xyz1 = xyz(kk, :) + (xyz1 - xyz(kk, :))*max(1,round(4-maxi)); % for better visibility when density is low
xyz2 = xyz(kk, :) + (xyz2 - xyz(kk, :))*max(1,round(4-maxi)); % for better visibilitywhen density islow
tt   = [xyz1; xyz2];
plot3(tt(:,1), tt(:,2), tt(:,3), 'Color', punCo,'MarkerSize',maSi+4*max(2,round(4-maxi)),'Marker','.'); % for better visibilitywhen density islow
end
end
